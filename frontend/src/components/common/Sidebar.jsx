...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { to: '/awards', icon: Trophy, label: 'Sổ Khen thưởng' },
    { to: '/profile', icon: User, label: 'Hồ sơ năng lực' },
    { to: '/approvals', icon: Calendar, label: 'Hàng chờ Thẩm định' },
    { to: '/admin', icon: ShieldCheck, label: 'Quản trị Hệ thống' },
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex fixed left-5 top-5 bottom-5 w-[76px] bg-soft-sidebar dark:bg-soft-sidebarDark rounded-[32px] flex-col items-center justify-between py-6 z-30 shadow-soft-lg text-white">
        ...
        <div className="flex flex-col items-center gap-3">
          <div 
            ...
            className="w-12 h-12 rounded-2xl bg-white/10 hover:bg-white/20 flex items-center justify-center text-white cursor-pointer transition-all shadow-sm"
          >
      ...
      <nav className="lg:hidden fixed bottom-3 left-3 right-3 h-16 bg-soft-sidebar/95 dark:bg-soft-sidebarDark/95 backdrop-blur-md rounded-full flex items-center justify-around px-2 z-40 shadow-soft-lg text-slate-400">
        <NavLink
          to="/"
          className={({ isActive }) =>
            `p-2.5 rounded-full transition-colors ${isActive ? 'text-white bg-white/15' : 'hover:text-white'}`
          }
        >
        ...
        <NavLink
          to="/ai-forecast"
          className="w-11 h-11 -mt-4 rounded-full bg-gradient-to-tr from-brand-600 to-brand-400 text-white flex items-center justify-center shadow-lg shadow-brand-500/30 active:scale-95 transition-transform"
        >
        ...
        <button
          onClick={handleLogout}
          className="p-2.5 rounded-full hover:text-rose-400 transition-colors"
        >
          <LogOut className="w-5 h-5" />
        </button>
      </nav>
